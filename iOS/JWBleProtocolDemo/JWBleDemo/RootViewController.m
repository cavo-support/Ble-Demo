...
- (void)viewDidLoad {
    ...
    self.title = @"JWBle Protocol Demo";
    
    __weak __typeof(self)weakSelf = self;
    [[JWBleProtocol shareInstance] setConnectStatusChangeCallBack:^(ConnectStatus deviceConnectStatus) {

        [weakSelf.navigationController popToRootViewControllerAnimated:true];
        [weakSelf updateConnectionBtnTitle];

        if (deviceConnectStatus == ConnectStatus_Connect) {
        ...
        } else if (deviceConnectStatus == ConnectStatus_SyncSuccess) {
            NSString *name = [NSString stringWithFormat:@"name : %@",[JWBleProtocol shareInstance].connectionModel.deviceName];
            NSString *mac = [NSString stringWithFormat:@"mac : %@",[JWBleProtocol shareInstance].connectionModel.macAddress];
            NSString *deviceNumber = [NSString stringWithFormat:@"%@ : %d",NSLocalizedString(@"Hardware number", nil),[JWBleProtocol shareInstance].connectionModel.deviceNumber];
            NSString *power = [NSString stringWithFormat:@"%@ : %d%%",NSLocalizedString(@"Power", nil),[JWBleProtocol shareInstance].connectionModel.power];
            NSString *appVersion = [NSString stringWithFormat:@"versionName : %@",[JWBleProtocol shareInstance].connectionModel.versionName];
        ...
        } else if (deviceConnectStatus == ConnectStatus_SyncFailure) {
            weakSelf.textView.text = NSLocalizedString(@"Failed to obtain device information", nil);
            [weakSelf.view hideToastActivity];
        } else if (deviceConnectStatus == ConnectStatus_BondSuccess) {
            weakSelf.textView.text = NSLocalizedString(@"Successful binding, basic information is being set", nil);
            [weakSelf.view makeToastActivity:self];
        } else if (deviceConnectStatus == ConnectStatus_BondFailure) {
            [weakSelf showBondFailAlert];
            [weakSelf.view hideToastActivity];
        } else if (deviceConnectStatus == ConnectStatus_DisConnect) {
