...
- (void)clickScanBtn {
    ...
    [[JWBleProtocol shareInstance] scanDeviceWithCallBack:^(JWBleProtocolModel *protocolModel) {
        ...
        if (protocolModel.per.RSSI.integerValue == 127 || protocolModel.per.name == NULL) {
            return;
        }
        
        BOOL hasNew = false;
        if (protocolModel.per.RSSI.intValue > (-weakSelf.filterRecordModel.rssi)) {
            if (weakSelf.filterRecordModel.name == nil || [weakSelf.filterRecordModel.name isEmpty]) {
                hasNew = true;
            } else if ([[protocolModel.per.name uppercaseString] containsString:[weakSelf.filterRecordModel.name uppercaseString]]) {
                hasNew = true;
            }
        }
        
        if (hasNew) {
            [weakSelf.scanDeviceArr addObject:protocolModel];
            
            //排序
            NSArray *result = [weakSelf.scanDeviceArr sortedArrayUsingComparator:^NSComparisonResult(CBPeripheral* obj1, CBPeripheral* obj2) {
                int rssi1 = obj1.RSSI.intValue;
                int rssi2 = obj1.RSSI.intValue;
                return rssi2 > rssi1;//[obj2.RSSI.longValue compare:obj1.RSSI.longValue]; //降序
            }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.protocolModel = self.scanDeviceArr[indexPath.row];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [[JWBleProtocol shareInstance] connectDevice:self.scanDeviceArr[indexPath.row]];
